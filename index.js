...
async function startSesi() {
    ...
    const x = require("./allxddev").makeWASocket(connectionUpdate)
    ...
    x.ev.on("connection.update", ({ connection }) => {
      if (connection === "open") {
        console.log("KONEKSI AllxdDev" + "Terhubung (" + x.user?.["id"]["split"](":")[0] + ")")
      }
      if (connection === "close") {
        startSesi()
      }
      if (connection === "connecting") {
        if (x.user) {
          console.log("SC PAIRING BY AllxdDev\nSELAMAT MENIKMATI")
        } else if (!useQR && !useCODE) {
          console.log("CONNECTION AllxdDev" + "Autentikasi Dibutuhkan\nGunakan Perintah \x1B[36mnpm start\x1B[0m untuk terhubung menggunakan nomor telepon\n\n\x1B[1m\x1B[41m Full Tutorial Chat wa.me/6285664731916 \x1B[0m\n\n")
        }
      }
    })
